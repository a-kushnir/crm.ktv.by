...
function calc_total_requests($requests, $date, $user) { 
  ...
  foreach($requests as $request)
    if ($date == $request['actual_date'] && $user == $request['updator'])
      $count++;
  return $count;
}
?>
...
foreach($requests as $request) {  
  if ($prev_date != $request['actual_date'] || $prev_user != $request['updator']) {
    $count = calc_total_requests($requests, $request['actual_date'], $request['updator']);
    echo '<thead>'.
      '<th colspan="8">'.$request['updator'].', '.human_date($request['updated_at']).' ('.$count.')</th>'.
      '</thead><tbody>';
    $prev_date = $request['actual_date'];
    $prev_user = $request['updator'];
  }
